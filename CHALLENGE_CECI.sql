...
WITH cost_by_product as (
SELECT p.product_code , p.product, sum( m.manufacturing_cost) as cost_
FROM gdb023.dim_product  p
JOIN gdb023.fact_manufacturing_cost m USING (product_code)
GROUP BY   p.product, p.product_code)


SELECT product_code , product, cost_
FROM cost_by_product
WHERE cost_ = ( SELECT max(cost_) from cost_by_product  )
OR cost_ = ( SELECT min(cost_) from cost_by_product )  
ORDER BY cost_ DESC
;
